...
INSERT INTO public.modules (id, course_id, title, description, order_index) VALUES
  ...
  ('b0000000-0000-4000-8000-000000000034', 'a0000000-0000-4000-8000-000000000004', 'Security Program Management and Oversight', 'Security+ domain 5: Policies, compliance, awareness, and third-party risk.', 4);
...
**Security controls** mitigate risk: preventive, detective, corrective, deterrent, and compensatory. **Cryptography** provides confidentiality (encryption), integrity (hashing, HMAC), and non-repudiation (digital signatures). Symmetric encryption uses one key; asymmetric uses public/private key pairs. TLS uses both for secure communication. You will use them in every security role.
...
**Security architecture** considers secure network design, segmentation, and defence in depth. **Zero trust** assumes no implicit trust; verify every access. **Resilience** includes redundancy, backups, and recovery procedures. **Cloud** and **hybrid** environments extend these concepts. CISSP and CISM build on this at an advanced level.
...
**Logging** and **monitoring** provide visibility into events. A **SIEM** (Security Information and Event Management) aggregates and analyses logs to detect threats. **Security operations** include managing incidents, responding to alerts, and maintaining controls. This prepares you for the Practitioner course (SOC, CySA+, SSCP) where you will use SIEMs and runbooks daily.
...
**Symmetric encryption** (e.g. AES) uses one shared key; **asymmetric** (e.g. RSA) uses public/private key pairs. **TLS** combines both: key exchange is asymmetric, bulk data encryption is symmetric. **Certificates** bind public keys to identities; **PKI** and **CAs** underpin trust. You will encounter TLS in every web server, VPN, and secure API. Security+ 1.2 and ISC² CC (Security Principles) cover these topics.

## Key Takeaway

Encryption at rest and in transit is baseline. Understanding how TLS and certificates work helps you configure and troubleshoot secure services and explain controls to others.
$md$, 26, 3),
...
**CVE** (Common Vulnerabilities and Exposures) gives each known vulnerability a unique ID. **CVSS** (Common Vulnerability Scoring System) scores severity (e.g. 0–10). Prioritisation considers score, **exposure** (internet-facing?), **exploitability**, and **asset criticality**. Not every high CVSS issue is urgent if the asset is isolated or non-critical. Security+ 2.3–2.4 and vulnerability management objectives align here.

## Key Takeaway

Use CVE/CVSS as input, not the only decision factor. Combine severity with business context to decide what to patch first.
$md$, 22, 3),
...
**Security baselines** (e.g. CIS Benchmarks) define secure configuration for OS and applications. **Hardening** removes unnecessary services, restricts permissions, and applies patches. **Secure SDLC** integrates security into design, development, and deployment (e.g. secure coding, dependency checks, secure pipelines). Security+ domains 3–4 cover secure host and network deployment; this lesson ties architecture to operations.

## Key Takeaway

Default installs are rarely secure. Baselines and hardening reduce attack surface; integrating security into the lifecycle prevents many issues before production.
$md$, 25, 3),
  ('b0000000-0000-4000-8000-000000000034', 'Security Policies and Compliance', 'Policies, standards, and compliance (Security+ domain 5).', $md$
# Security Policies and Compliance

**Security policies** set organisational expectations (e.g. acceptable use, access control). **Standards** and **procedures** implement them. **Compliance** (GDPR, HIPAA, PCI-DSS, SOC 2) drives many controls; audits verify adherence. Security+ domain 5 (Security Program Management and Oversight) and ISC² CC (governance) cover policy and compliance. Understanding these helps you justify controls and work with auditors.

## Key Takeaway

Policies and compliance are not optional in most organisations. They define the “why” behind many technical controls you will implement and operate.
$md$, 24, 1),
...
**Security awareness training** teaches users to recognise phishing, use strong passwords, and handle data safely. **Phishing simulations** measure behaviour and reinforce training. **Third-party risk** and **vendor management** address suppliers and partners who have access to systems or data; contracts, assessments, and monitoring reduce risk. Security+ 5.3–5.4 and ISC² CC (Security Program) align. People and partners are part of the security perimeter.

## Key Takeaway

Technical controls are not enough. Awareness reduces human error; third-party risk management extends your control to the supply chain.
$md$, 22, 2);
...
The **vulnerability management lifecycle** includes discovery (scanning), prioritisation (risk scoring), remediation (patching or mitigating), and verification. **CVSS** and organisational context help prioritise. Effective vuln management reduces the attack surface and is a core responsibility in many security roles.
...
**Incident response** follows phases: preparation, detection and analysis, containment, eradication, recovery, and post-incident activity. **Containment** limits damage; **eradication** removes the threat; **recovery** restores systems. **Communication** and **documentation** are critical. This is essential for SOC and IR roles.
